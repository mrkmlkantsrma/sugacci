...
<div class="form-group"> 
    ...
    <textarea name="meta_tags" id="meta_tags" rows="2" class="form-control"><?= $product['meta_tags']; ?></textarea>
</div>

<div class="form-group"> 
    <label>Product Basic Info & meta Description</label>
    <textarea name="basic_description" id="basic_description" rows="2" class="form-control"><?= $product['basic_description']; ?></textarea>
</div>
...
<div class="form-group for-shop">
    ...
    <div class="col-md-6">
        <label>Price</label>
        <input type="text" name="price" placeholder="without currency at the end" value="<?= $product['price']; ?>" class="form-control">
    </div>
    <div class="col-md-6">
    </div>
    </div>
...
<div class="row">
    <div class="col-md-6">
        ...
        <input type="text" name="old_price" placeholder="without currency at the end" value="<?= $product['discount_price']; ?>" class="form-control">
    </div>
...
<div class="form-group bordered-group">
    ...
    if (isset($product['image']) && $product['image'] != null) {
        $image = 'attachments/shop_images/'. htmlspecialchars($product['image']);
                ...
                <td class="deleteOuter">
                    <?php if( isset( $productImages)  ){
                        ...
                        <div class="jqImageContainer">
                            <img src="<?php echo base_url();?>attachments/shop_images/<?php echo $img->image_name ?>" width="100" />
                            <a class="jqDeleteImage" href="javascript:void(0)" data-imageId="<?php echo $img->id?>" data-galleryType="Perfume"><i class="fa fa-times" aria-hidden="true"></i></a>
                        </div>
                    <?php }
                    } ?>
                </td>
    ...
    <div class="col-md-6">
        ...
        <select class="selectpicker form-control show-tick show-menu-arrow" name="shop_categories">
            <?php foreach ($shop_categories as $shop_categorie) { 
                $selected = '';
                if($product['shop_categorie'] == $shop_categorie['id']){
                    $selected = 'selected';
                } ?>
                <option <?= isset($_POST['shop_categorie']) && $_POST['shop_categorie'] == $shop_categorie['id'] ? 'selected' : '' ?> value="<?= $shop_categorie['id'] ?>" <?= $selected ;?>><?= $shop_categorie['name'] ?></option>
            <?php } ?>
        </select>
    </div>
    <div class="col-md-6">
        ...
        <?php $male = ''; $female = ''; $both = '';
            ...
            <input type="radio" class="btn-check" name="gender" id="btnradio1" value="male" autocomplete="off" <?= $male; ?> >
            <label class="btn btn-outline-primary" for="btnradio1">Male</label>

            <input type="radio" class="btn-check" name="gender" id="btnradio2" value="female"  autocomplete="off" <?= $female; ?>>
            <label class="btn btn-outline-primary" for="btnradio2">Female</label>

            <input type="radio" class="btn-check" name="gender" id="btnradio3" value="both"  autocomplete="off" <?= $both; ?>>
...
<div class="form-group for-shop">
    ...
    <div class="col-md-6">
        ...
        <input type="text" placeholder="number" name="quantity" value="<?= $product['quantity']; ?>" class="form-control" id="quantity">
    </div>
    <div class="col-md-6">
        <?php $one = ''; $two = ''; $three = ''; $four = ''; $fifth = '';
                if($product['rating'] == '1'){
                    $one = 'checked';
                }
                if($product['rating'] == '2'){
            ...
            <div class="btn-group ratingChoice" role="group" aria-label="Basic radio toggle button group" >
                <input type="radio" class="btn-check" name="rating" id="ratingradiobtn1" value="1" autocomplete="off" <?= $one; ?>>
                <label class="btn btn-outline-primary" for="ratingradiobtn1">1</label>

                <input type="radio" class="btn-check" name="rating" id="ratingradiobtn2" value="2"  autocomplete="off" <?= $two; ?>>
                <label class="btn btn-outline-primary" for="ratingradiobtn2">2</label>

                <input type="radio" class="btn-check" name="rating" id="ratingradiobtn3" value="3"  autocomplete="off" <?= $three; ?>>
                <label class="btn btn-outline-primary" for="ratingradiobtn3">3</label>

                <input type="radio" class="btn-check" name="rating" id="ratingradiobtn4" value="4"  autocomplete="off" <?= $four; ?>>
                <label class="btn btn-outline-primary" for="ratingradiobtn4">4</label>

                <input type="radio" class="btn-check" name="rating" id="ratingradiobtn5" value="5"  autocomplete="off" <?= $fifth; ?>>
    ...
    <div class="col-md-6">
        ...
        <input type="text" placeholder="Position number" name="position" value="<?= $product['position']; ?>" class="form-control">
    </div>
    <div class="col-md-6">
    </div>
    </div>
...
$(document).ready(function(){
        ...
        // Roll Images Reordering
            ...
            $(document).on('click', '#arrangedImg', function(){  
                ...
                $.ajax({
                    url: `<?php echo base_url();?>home/HomeController/product/reArrangeGalleryImage`,
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        row_id : rowID,
        ...
        $(document).on('click', '.jqDeleteImage', function(){  
            ...
            var obj = $(this);
            $.ajax({
                url: `<?php echo base_url();?>admin/ecommerce/Product/deleteProductGalleryImage`,
                type: 'post',
                data: {
                    imageId:imageId,
                },
